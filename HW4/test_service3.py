...
def test_get_listing_nested_resources(section, resource):
    ...
    assert response.status_code == 200 and section, resource in response.text
...
def test_get_creating_resource(title, body, userId):
    ...
    assert response.status_code == 200 and title, body in response.text and str(userId) in response.text
...
def test_updating_resource(id):
    ...
    assert response.status_code == 200 and str(id) in response.text